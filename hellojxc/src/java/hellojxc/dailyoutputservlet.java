 ...
 * @author marsday
 */
@WebServlet(name = "dailyoutputservlet", urlPatterns = {"/listdailyoutput","/adddailyoutput","/deldailyoutput","/updatedailyoutput","/getdailyoutput"})
public class dailyoutputservlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(uri.endsWith("/getdailyoutput")) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(!Utility.checkSession(request, response))
            return;
        String uri = request.getRequestURI();
        if (uri.endsWith("/listdailyoutput")) {
            //return JSON
            listoperation(request,response);
        }else if(uri.endsWith("/adddailyoutput")) {
            addoperation(request,response);
        }else if(uri.endsWith("/updatedailyoutput")) {
            updateoperation(request,response);
        }else if(uri.endsWith("/deldailyoutput")) {
    ...
    private void deloperation(HttpServletRequest request, HttpServletResponse response)
        ...
        if(names.length > 0)
        {
            String sql = "update jxc_next_daily_output set del_flag = 1 where id in (";
            int index = 0;
       ...
       response.sendRedirect("index.html?function=listdailyoutput");
    } 
    ...
    private void updateoperation(HttpServletRequest request, HttpServletResponse response)
        ...
        String sql = "update jxc_next_daily_output set target_id = " + target_id +","
                                                        + " pay_id= " + pay_id + ","
                                                        + " title= " + "'" + title + "'," 
                                                        + " price= " + price +"," 
    ...
    private void addoperation(HttpServletRequest request, HttpServletResponse response)
        ...
        String sql = "insert into jxc_next_daily_output (title,target_id,pay_id,price,operationtime,recordtime,refer) values(" 
                +  "'" +  title  + "',"   
                +  target_id + "," 
                +  pay_id + ","
                +  price  + ","
    ...
    private void getoperation(HttpServletRequest request, HttpServletResponse response)
        ...
        //}
        String sql = "select id, title, target_id, pay_id, price, operationtime, recordtime, refer from jxc_next_daily_output where id= " + "'" + input + "'";
